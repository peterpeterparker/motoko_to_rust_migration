...
fn post_upgrade() {
    ...
    // let (user_bytes, entries_bytes) = from_bytes(bytes).map(|c| c.0)?;
    // let user: String = Decode!(&user_bytes, {user: String}).map_err(|err| err.to_string());
    // let entries: Vec<(String, Asset)> = Decode!(&user_bytes, Vec<(String, Asset)>).map_err(|err| err.to_string());
}
